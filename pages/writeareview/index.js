              ...
              {talks.length === 0 ? null : talks.slice(0,10).map((item,index) => 
                  <div className="row border-bottom pb-2 py-2" key={index}>
                    <div className="position-relative col-2 p-0 card card-body">
                        <Image 
                        src={`${item.logo_url === null ? "/images/user_medium_square.png" :  item.logo_url.replace("http://","https://")}`}
                        className="img-fluid p-0 rouned"
                        loader={() => item.logo_url === null ? "/images/user_medium_square.png" :  item.logo_url.replace("http://","https://")}
                        alt=""
                        layout="fill"
