...
const Dashboard = ({gems,cultures,categories}) => {
  ...
  const handleFind = () => {
   ...
   if(category !== 0 && culture !== 0 &&  category.length != 0 && culture.length != 0){
       ...
       const cultureId =  culture !== 0 && cultures.data.find((item) => item.name == culture).id;

       const categoryId = category !== 0 && categories.data.find((item) => item.name == category).id;
                  ...
                  <div className="border rounded">
                    <div className="w-100 position-relative">
                       ...
                       <Image 
                          ...
                          loader={() => item.logo_url === null ? "/images/db-img.jpg?w=500" :  item.logo_url.replace("http://","https://") + "?w=500"}
                          alt=""
                          height={300}
                          width={500}
                          layout='responsive'
